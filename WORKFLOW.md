...
Follow-up reviews should use `REVIEW_MODE=gate` or `REVIEW_MODE=confirm-fixes`: verify addressed findings, blockers, regressions, and newly introduced high-value issues. They should not restart a broad hunt for every low advisory. If the runner cannot find previous review state in the living PR comment or `PR_LOG_PATH`, these focused modes must fail closed with `needs_human` instead of approving an empty confirmation. Low-severity advisory findings under an `approve` verdict are useful signal, but they are not automatically merge blockers unless the product repo or human gate says so.

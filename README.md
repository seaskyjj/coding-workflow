...
A PR diff shows only the changed hunks, but a proposal's argument lives in the whole document. So for a **PR** proposal review the runner auto-fetches the **full current text of each changed doc file** (markdown/text: `.md`/`.mdx`/`.markdown`/`.txt`/`.rst`/`.adoc`) at the PR head and appends it as a `FULL CHANGED-DOCUMENT CONTEXT` block, so the reviewer reconstructs the argument from the whole doc instead of over-confidently judging a partial hunk. The doc context is bounded by `PROPOSAL_DOC_CONTEXT_CHARS` (default `min(120000, MAX_DIFF_CHARS)`) **and shares the diff's budget** — the runner shrinks the doc cap by the diff size so the composed (diff + docs) prompt stays within `MAX_DIFF_CHARS`, never a second cap stacked on top. Any changed doc that doesn't fit, was deleted, or isn't fetchable is listed under `CHANGED DOCUMENTS NOT INCLUDED IN FULL` **and forces `needs_human` at the runner** (like an omitted code-file patch) — the model can't approve a partial argument even if it tries. The prompt also fails closed when a load-bearing cited source/metric definition lives *outside* the changed docs (another file, an external link, a dataset) and isn't reproduced. (`--diff-file` has no GitHub to fetch from, so pass the whole document yourself.) The argument shape the reviewer reconstructs is chosen per artifact type — investigation/fix vs decision/ADR vs knowledge/methodology — not forced into one template.

...
- For a PR review the runner appends a **FULL CHANGED-DOCUMENT CONTEXT** block: the current full text of the changed docs (the DIFF above is only the changed hunks). When present, reconstruct the argument from that full text, not from the hunks alone — do not fail closed merely because the diff was partial. Still fail closed when context is genuinely missing: any doc listed under **CHANGED DOCUMENTS NOT INCLUDED IN FULL**, or a load-bearing cited source/evidence/metric definition that lives **outside** the changed docs (e.g. another file, an external link, a dataset) and is not reproduced here.
- If you are given a diff/excerpt rather than the whole artifact and the changed portion cannot be reconstructed without surrounding context (definitions, options, cited evidence, metric definitions not present) — and that context was not supplied in a full-document block — do NOT approve: return `needs_human` (or a finding plus a `could_not_verify` entry) asking for full-document context. A partial argument must not be approved, just as a truncated code diff is never approved.
